...
<table>
  <tr>
    <td>
        <img height="200" alt="image" src="https://user-images.githubusercontent.com/8360880/185859218-9b80ef50-4c8c-486f-baf0-8b846d4feedf.png">
    </td>
    <td>
        <img height="200" alt="image" src="https://user-images.githubusercontent.com/8360880/185860747-0651f112-3e3d-4921-ac04-09aad16ddcb0.png">
    </td>
    <td>
        <img height="200" alt="image" src="https://user-images.githubusercontent.com/8360880/185861069-e9da1fe8-6235-405a-aa04-e7b29b1ad5a3.png">
    </td>
   </tr> 
</table>
...
In the first version I used Redis.OM to store most of the statistics and incident data. After a while I noticed it's a bit hard to browse the data directly using RedisInsight since the keys are automatically generated by Redis.OM. So in this new version I decided to drop the use of Redis.OM for my `StatisticsService` and instead use Redis's List, Set, and Hash to store the data.

As you can see in the image below, on the second version the data is stored neatly and we can visualize it as a directory, much easier to explore and change.

<table>
  <tr>
    <td>
        <p><b>First Version (V1)</b></p>
        <img width="400" alt="redis-v1" src="https://user-images.githubusercontent.com/8360880/187242959-f4e6e380-071b-49d1-8a7b-be3ebf507e77.png">
    </td>
    <td>
        <p><b>Second Version (V2)</b></p>
        <img width="400" alt="redis-v2" src="https://user-images.githubusercontent.com/8360880/187242971-109265e0-da56-452e-bbc2-f787039e7d8f.png">
    </td>
   </tr> 
</table>
...
- `SET STATISTICS:FIRST_STARTUP "2022-08-22T15:38:06.0797433+07:00"`
...
#### How the data is stored (V2)

For each service uptime status snapshot, it will push several new items to the following lists ({0} = service name, {1} = date in format YYYY-MM-DD)

- `HSET STATISTICS:LAST_STATUS "Prometheus" "1"`
- `SADD STATISTICS:SERVICES "Prometheus"`
- `RPUSH STATISTICS:{0}:TIMESTAMPS:{1} "2022-08-22T15:38:06.0797433+07:00"`
- `RPUSH STATISTICS:{0}:HEALTH:{1} "1"`
- `RPUSH STATISTICS:{0}:LATENCY:{1} "12"`
...
#### How the data is accessed (V2)

To get the chart data for latecy graph, we can simply retrieve all the list values.

- `LRANGE STATISTICS:{0}:TIMESTAMPS:{1} 0 -1`
- `LRANGE STATISTICS:{0}:HEALTH:{1} 0 -1`
- `LRANGE STATISTICS:{0}:LATENCY:{1} 0 -1`
